    ...
    function subscribeToTopics(topics, callback) {
        ...
        mqttClient.subscribe(topics, options, function (error) {
            if (error) {
                ...
                config.getLogger().info('Successfully subscribed to the following topics:\n%j\n', topics);
                if (callback) {
                    callback(null);
                }
            }
...
function generateCommandExecution(apiKey, device, attribute) {
    ...
    config
        ...
        .info(
            context,
            'Sending command2 execution to [%s] with payload [%s] and mqtt options [%j]',
            commandTopic,
            payload,
            options
        );
...
function sendConfigurationToDevice(apiKey, deviceId, results, callback) {
    ...
    if (mqttClient.connected) {
        ...
        config
            ...
            .error(
                context,
                'Configuration:\n %j was not sent to the device %s due to not connected',
                payload,
                commandTopic
            );
    }
